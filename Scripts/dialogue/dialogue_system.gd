...
func _choice_resource(i: DialogueChoice) -> void:
	...
	for item in i.choice_text.size():
		...
		if function_resource:
			...
			DialogueButtonVar.connect("pressed",
			_choice_button_pressed.bind(get_node(function_resource.target_path), function_resource.function_name).bindv(function_resource.function_arguments),
			CONNECT_ONE_SHOT)
		else:
...
func _choice_button_pressed(target_node: Node, wait_for_signal_to_continue: String):
	...
	#button press audiostreamplayer here
...
func _text_resource(i: DialogueText) -> void:
	...
	while dialogue_label.visible_characters < total_characters:
		...
		if character_timer >= (1.0 / i.text_speed) or text_without_square_brackets[dialogue_label.visible_characters] == " ":
			...
			dialogue_label.visible_characters += 1
			if character != " ":
				$AudioStreamPlayer.pitch_scale = randf_range(i.text_volume_pitch_mix, i.text_volume_pitch_max)
				$AudioStreamPlayer.play()
				if i.speaker_img_Hframe != 1:
					if speaker_sprite.frame < i.speaker_img_Hframe - 1:
